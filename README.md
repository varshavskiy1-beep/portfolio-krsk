...
Публичный сайт: https://portfolio-krsk.vercel.app — production только через Vercel с ветки `master`. Ветка `gh-pages` и workflow GitHub Pages больше не используются (Vercel Preview для `gh-pages` отключён в `vercel.json`).
